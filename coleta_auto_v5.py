"""
AgroIA-RMC — Coleta Automática V5
=================================
Versão que re-navega para a página correta após voltar do detalhe.

Execute: python coleta_auto_v5.py
"""
import os
import re
import time
...
def ir_para_pagina(page, num_pagina):
    ...
    if atual and atual.inner_text().strip() == str(num_pagina):
        return True
    
    # Clicar nos números até chegar na página desejada
    for _ in range(num_pagina):
        atual = page.query_selector("td.rich-datascr-act")
        pag_atual = int(atual.inner_text().strip()) if atual else 1
        
        if pag_atual == num_pagina:
            return True
        
        # Encontrar próxima página para clicar
        tds = page.query_selector_all("td.rich-datascr-inact")
        clicou = False
        for td in tds:
            num = td.inner_text().strip()
            if num.isdigit() and int(num) > pag_atual:
                td.click()
...
def main():
    ...
    print("=" * 60)
    print("AgroIA-RMC — COLETA AUTOMÁTICA V5")
    print("=" * 60)
    ...
    with sync_playwright() as p:
        ...
        print(f"\n[4] Coletando ({max_paginas} páginas, Ctrl+C para parar)...\n")
        
        pagina = 1
        
        while not interrompido and pagina <= max_paginas:
            # Navegar para a página correta
            ir_para_pagina(page, pagina)
            ...
            # Processar cada licitação pendente na página
            for idx, proc in enumerate(pendentes_pagina):
                ...
                try:
                    # Garantir que estamos na página correta
                    ir_para_pagina(page, pagina)
